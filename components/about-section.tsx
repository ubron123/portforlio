...
export function AboutSection({ sectionGradient }: AboutSectionProps) {
  ...
  const threshold = 100 // Amount of scroll needed to trigger slide change
  ...
  const handleWheel = useCallback((e: WheelEvent) => {
    ...
    // Check if section fills the viewport (is in "locked" position)
    ...
    if (isInView) {
      ...
      else if (e.deltaY < 0 && activeSlide === "certifications") {
        e.preventDefault()
      ...
      // - On "certifications" and scrolling down (continue to next section)
      // - On "whoami" and scrolling up (go back to previous section)
    }
  }, [activeSlide])

  // Check if section is in view for locking state
  const handleScroll = useCallback(() => {
    ...
    setIsLocked(isInView)

    // Reset to whoami when scrolling back up past the section
    if (rect.top > viewportHeight * 0.5 && activeSlide === "certifications") {
      setActiveSlide("whoami")
      accumulatedDelta.current = 0
    }
  }, [activeSlide])
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-12 md:gap-8">
                ...
                <div className="space-y-6 md:pr-8">
                  ...
                  <div className="flex items-center gap-6 pt-4">
                    ...
                    <a
                      href="#certifications"
                      className="text-sm transition-colors flex items-center gap-2 text-white/50 hover:text-white/70"
                    >
                      <span className="text-white/30">{">"}</span> My Certifications
                    </a>
                  </div>
                </div>
                ...
                <div className="space-y-6 md:pr-8">
                  ...
                  <div className="flex items-center gap-6 pt-4">
                    <a
                      ...
                      className="text-sm transition-colors flex items-center gap-2 text-white/50 hover:text-white/70"
                    >
                      <span className="text-white/30">{">"}</span> Who am I
                    </a>
                    <a
                      href="#certifications"
                      className="text-sm transition-colors flex items-center gap-2 text-cyan-400 font-bold"
